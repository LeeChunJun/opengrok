...
.help-content { font-size: 14px; color: var(--fg); line-height: 1.6; max-width: 920px; }
.help-content h1 { font-size: 22px; font-weight: 600; margin: 8px 0 16px; }
.help-content h2 { font-size: 18px; font-weight: 600; margin: 24px 0 8px; color: var(--fg); }
.help-content h3, .help-content h4 { font-size: 15px; font-weight: 600; margin: 18px 0 8px; color: var(--fg); }
.help-content h5 { font-size: 14px; font-weight: 600; margin: 14px 0 6px; color: var(--muted); }
.help-content p { margin: 8px 0; }
.help-content pre.example, .help-content pre {
  background: var(--bg); border: 1px solid var(--border); border-radius: 6px;
  padding: 14px 16px; font-size: 12.5px; font-family: var(--font-mono);
  overflow-x: auto; line-height: 1.55; color: var(--fg);
  white-space: pre-wrap; word-break: break-word;
}
.help-content code { background: var(--bg); padding: 1px 5px; border-radius: 3px;
...
.help-content img { max-width: 100%; border: 1px solid var(--border); border-radius: 6px;
  margin: 8px 0; }
.help-content ul, .help-content ol { padding-left: 24px; margin: 8px 0; }
.help-content li { margin: 4px 0; }
</style>

<%-- help.jsp start

    help.jsp -- OpenGrok help page. Uses pageheader.jspf + foot.jspf for
    the new UI chrome (no mast.jsp; help is a static page that does not
    need the path-based compact-nav / breadcrumb).
--%>
<%
{
    PageConfig _chromeHelpCfg = PageConfig.get(request);
    _chromeHelpCfg.checkSourceRootExistence();
    _chromeHelpCfg.setTitle("OpenGrok Help");
}
%>
...
<main class="container">
    ...
    <pre class="example">
        ...
        <a href="search?<%= QueryParameters.FULL_SEARCH_PARAM_EQ %>&amp;<%= QueryParameters.DEFS_SEARCH_PARAM_EQ %>setResourceMonitors"><%= QueryBuilder.DEFS %>:setResourceMonitors</a>

        To find files that use sprintf in usr/src/cmd/cmd-inet/usr.sbin/:
        <a href="search?<%= QueryParameters.REFS_SEARCH_PARAM_EQ %>sprintf&amp;<%= QueryParameters.PATH_SEARCH_PARAM_EQ %>usr%2Fsrc%2Fcmd%2Fcmd-inet%2Fusr.sbin%2F"><%= QueryBuilder.REFS %>:sprintf <%= QueryBuilder.PATH %>:usr/src/cmd/cmd-inet/usr.sbin</a>
        ...
        <a href="search?<%= QueryParameters.FULL_SEARCH_PARAM_EQ %>pstack&amp;<%= QueryParameters.PATH_SEARCH_PARAM_EQ %>Makefile">pstack <%= QueryBuilder.PATH %>:Makefile</a>
        ...
        <a href="search?<%= QueryParameters.FULL_SEARCH_PARAM_EQ %>-%22%2Fusr%2Fbin%2Fperl%22+%2B%22%2Fbin%2Fperl%22">-"/usr/bin/perl" +"/bin/perl"</a>
        ...
        <a href="search?<%= QueryParameters.PATH_SEARCH_PARAM_EQ %>/ma[a-zA-Z]*/"><%= QueryBuilder.PATH %>:/ma[a-zA-Z]*/</a>

        To find all main methods in all files analyzed by C analyzer (so .c, .h, ...) do:
        <a href="search?<%= QueryParameters.FULL_SEARCH_PARAM_EQ %>main&amp;<%= QueryParameters.TYPE_SEARCH_PARAM_EQ %>c">main <%= QueryBuilder.TYPE %>:c</a>
    </pre>

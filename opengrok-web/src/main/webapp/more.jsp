...
<%
{
	PageConfig _moreCfg = PageConfig.get(request);
	_moreCfg.checkSourceRootExistence();
}
%>
...
{
	PageConfig _moreCfg = PageConfig.get(request);
	File resourceFile = _moreCfg.getResourceFile();
	String path = _moreCfg.getPath();
	RuntimeEnvironment env = _moreCfg.getEnv();
	...
	if (activeProject == null) {
		qbuilder = _moreCfg.getQueryBuilder();
	} else {
		searchHelper = _moreCfg.prepareInternalSearch(SortOrder.RELEVANCY);
		/*
	...
	try {
		...
		if (tquery != null) {
%><p>
	<span class="pagetitle">Lines Matching <span class="bold"><%=tquery%></span></span>
</p>
<div id="more" style="line-height: 1.5em;">
	<pre>
		...
		if (docId >= 0) {
			didPresentNew = searchHelper.getSourceContext().getContext2(env, searchHelper.getSearcher(), docId, out, xrefPrefix,
			null, false, tabSize);
		}
		if (!didPresentNew) {
			/**
			...
			try (Reader r = IOUtils.createBOMStrippedReader(new FileInputStream(resourceFile), StandardCharsets.UTF_8.name())) {
				sourceContext.getContext(r, out, xrefPrefix, null, path, null, false, false, null, null);
			}
		}
		%>
	</pre>
